            ...
            @if($sellers->count!=null)
                    @if($sellers->sellingType=="For Sale")
                        @if($sellers->sellingStatus==null)
                            ...
                            <!-- (<font size="2">No. of Potential Buyers <a data-toggle="modal" data-target="#intentions_modal" class="property_owned"></font><font size="2" color="blue"><b> {{$sellers['count']}}</b></a></font>) -->
                        @endif
                    @else
                        @if($sellers->sellingStatus==null)
                            (<font size="2">No. of Interested Leasers <a href="{{url('/viewuserwhointended/'.$sellers['tid'].'/list')}}"><b> {{$sellers['count']}}</b></a></font>)
                            <!-- (<font size="2">No. of Potential Leasers <a data-toggle="modal" data-target="#intentions_modal" class="property_owned"></font><font size="2" color="blue"><b> {{$sellers['count']}}</b></a></font>) -->
                        @endif
                    @endif
                @endif
                @if($sellers->sellingStatus!="sold")
                    ...
                    <a id="delete" link="{{url('/deletePost/'.$sellers['tid'].'/delete')}}" data-toggle="modal" data-placement="left" title="DELETE" data-target="#deleteModal"><b><i class="fas fa-times-circle fa-lg" style="color:teal"></i></b></a>
                @endif
            </h6>
                ...
                @if($sellers->status=="intended" && $sellers->sellingStatus=="leased")
                    <div class="dropdown no-arrow">
                        <a href="{{url('/seller/'.$sellers['tid'].'/repost')}}" class="buttonImage view" data-toggle="tooltip" data-placement="left" title="REPOST PROPERTY"> <b><i class="fas fa-retweet-alt" style="color:teal"></i> </b></a>
                    </div>
                @endif
        </div>
        @if($sellers->sellingStatus!=null)
                    ...
                    <center>
                        <label class="col-md-11 col-form-label">        
                            <div id="makeMeScrollable" class="scrollableArea">
                                <img class="image" id="preview" src="{{ $image->fileExt }}" height="100" width="200"> 
                            </div>
                        </label>
                    </center>   
                    @endif
...
<div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">Are you sure to delete posted property?</div>
